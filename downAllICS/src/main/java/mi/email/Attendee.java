...
public class Attendee {
    ...
    public void populateFromEntries(ListIterator<String> iter) {
        ...
        if (line.contains("ATTENDEE")) {
            ...
            if(entry.contains("X-NUM-GUESTS=0:MAILTO:")) {
                String[] entries = entry.split("X-NUM-GUESTS=0:MAILTO:");
                mEmail = entries[1];

                String[] xx = entry.split("CN=");
                String[] yy = xx[1].split("X-NUM-GUESTS=0:MAILTO:");
                mName = yy[0];
            }
            else if(entry.contains("MAILTO:")) {
                String[] entries = entry.split("MAILTO:");
                mEmail = entries[1];

                String[] xx = entry.split("CN=");
                String[] yy = xx[1].split(":MAILTO:");
                mName = yy[0];
            }else if(entry.contains("mailto:")) {
                String[] entries = entry.split("mailto:");
                mEmail = entries[1];
